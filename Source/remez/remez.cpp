 ...
 */
   ...
   memset(foundExt,0,(2*r) * sizeof(int));
   k = 0;
...
short isDone(int r, int Ext[], double E[], int gridsize)
{
   int i;
   ...
   for (i=1; i<=r; i++)
   {
	  if ((Ext[i]>=gridsize)||(Ext[i]<0)) {
	     printf("Trouble! (%d > %d) : %d %x\n",Ext[i], gridsize,i,Ext[i]);
		 return 1;
	  }
